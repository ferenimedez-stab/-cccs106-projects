...
class WeatherServiceError(Exception):
    ...
    ft.AlertDialog(title = str(Exception),
    ...
    async def get_forecast(self, city: str):
        ...
        try:
            async with httpx.AsyncClient(timeout = self.timeout) as client:
                ...
                if response.status_code == 404:
                    raise WeatherServiceError(f"City '{city}' not found for forecast.")
                elif response.status_code != 200:
                    raise WeatherServiceError(f"Error fetching forecast data: {response.status_code}")

                return response.json()

        except httpx.TimeoutException:
            raise WeatherServiceError("Request timed out while fetching forecast.")
        except Exception as e:
